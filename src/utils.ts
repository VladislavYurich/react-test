...
export const printBtnClick = (
  ...
  if (signRef) {
    signRef.current.style.display = 'block'
  }
  if (areaRef) {
    areaRef.current.style.display = 'none'
  }
  printFunction(action, printElement)
  btnRef.current.style.display = 'flex'
  if (signRef) {
    signRef.current.style.display = 'none'
  }
  if (areaRef) {
...
const printFunction = (action: 'print' | 'save', printElement: any) => {
